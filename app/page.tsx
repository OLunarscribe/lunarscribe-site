...
/* =========================================================
   HERO VISUAL: Vertical “task list” slider (up/down)
   Matches the reference style: tabs + stacked items + mask
========================================================= */
...
function WorkflowListSlider() {
  const allTasks: TaskItem[] = [
    {
      title: "Payroll management",
      subtitle: "Due on 2nd July",
      icon: <DollarSign className="h-4 w-4 text-white/80" />,
      status: "waiting",
    },
    {
      title: "Employee Tracking",
      subtitle: "2 days ago",
      icon: <Users2 className="h-4 w-4 text-white/80" />,
      status: "done",
    },
    {
      title: "Social media post",
      subtitle: "Cancelled by user",
      icon: <CalendarDays className="h-4 w-4 text-white/80" />,
      status: "canceled",
    },
    {
      title: "Lead list",
      subtitle: "70% prepared",
      icon: <ListTodo className="h-4 w-4 text-white/80" />,
      status: "progress",
    },
    {
    ...
    },
    {
      ...
      icon: <Workflow className="h-4 w-4 text-white/80" />,
      status: "waiting",
    },
  ];
  ...
  // Layout constants (must match row height classes below)
  const ROW_H = 72; // px (visual row height)
  const VISIBLE_ROWS = 4;

  // Keep index in range when tab changes
  useEffect(() => {
    setIndex(0);
  }, [tab]);

  // Auto-scroll (up/down feel)
  useEffect(() => {
      ...
      <main className="relative z-10">
        <section className="mx-auto max-w-6xl px-6 pt-12 md:pt-16">
          <div className="grid items-center gap-10 md:grid-cols-2">
            ...
            {/* Visual (VERTICAL SLIDER like your image) */}
            <motion.div
              initial={{ opacity: 0, y: 14 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.55, delay: 0.1 }}
          ...
          <div className="grid gap-10 md:grid-cols-2">
            <div>
              ...
              <p className="mt-3 max-w-xl text-white/70">
                If your team lives in spreadsheets, Slack, and five tools that don’t agree, you don’t have a system.
                You have noise. Lunarscribe turns execution into a controlled, measurable workflow.
              </p>
              ...
              <Card className="rounded-3xl border-white/10 bg-white/5 text-white">
                ...
                <CardContent className="pt-0">
                  <div className="space-y-3 text-sm text-white/70">
                    {["Your workflow mapped into a clean model", "A dashboard walkthrough", "Export examples", "Implementation plan"].map(
                      (x) => (
                        <div key={x} className="flex items-start gap-2">
                          <Check className="mt-0.5 h-4 w-4 text-purple-200" />
                          <span>{x}</span>
        ...
        <footer className="border-t border-white/10 bg-black/20">
          <div className="mx-auto max-w-6xl px-6 py-10">
            ...
            <div className="mt-8 text-xs text-white/50">© {new Date().getFullYear()} Lunarscribe. All rights reserved.</div>
          </div>
        </footer>
      </main>
    </div>
